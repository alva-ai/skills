...
| Automation / Push | A new user-facing recurring automation defaults to delivery even without explicit alert wording. Read [alva-knowledge.md](alva-knowledge.md) before design, build or modify a feed that declares actionable outputs with `alertOutput(typeDoc)`, then follow [push-notifications.md](push-notifications.md) to verify its publisher, subscription, and delivery paths. |
