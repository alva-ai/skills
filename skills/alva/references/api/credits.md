...
Run `alva account credits --help` first; it is the command contract. Use this surface
when the user asks how many credits they have, how many credits they used, or
which recent operations consumed credits.

## Scope

`alva account credits` is viewer-scoped. It queries only the authenticated user's wallet
...
```bash
alva account credits items --today --first 20
alva account credits items --last 7d --first 50
...
```bash
alva account credits items --last 7d --session-id <session_id> --first 50
```

For pagination, pass the previous response's `items.pageInfo.endCursor`:

```bash
alva account credits items --last 30d --first 50 --after '<cursor>'
```
