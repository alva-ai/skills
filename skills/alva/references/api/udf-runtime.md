...
Use the native write/edit tools to place the entry script at the absolute ALFS
path, then use the `alva` tool for registration and invocation. Pass the JSON
Schema inline with `--params-schema`.

```bash
alva functions register --playbook-id <playbook-id> --function-name analyze --entry-script-path '/alva/home/<username>/playbooks/<name>/udf/analyze.js' --params-schema '<json-schema>' --no-allow-charges
...
- Build nontrivial schemas as structured JSON, serialize them once, and pass
  the result through inline `--params-schema`. The schema must match both UI
