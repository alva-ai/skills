...
| explicitly publish/create a Thesis from a viewpoint, update/close/delete an existing Thesis, explicitly polish a viewpoint | Thesis | Read [thesis.md](references/thesis.md). Before `create`, show the final Thesis text and ask the user to confirm it; after authoritative readback, emit one `<thesis-preview>` XML block with verified identity, author, exact body, and entity IDs/tickers/icons; rewriting is a separate explicit action. |
