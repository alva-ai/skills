...
| ticker read, analyze a named ticker or company, past-hour tracking, "why did it move", investor focus, recent catalysts, unusual move     | Financial Analysis + Platform Data: Ticker Read | Treat the five official methods as first-tier sources; read [ticker-read.md](references/ticker-read.md) before source selection, starting with `alva/company-anomaly-read` for intraday/hourly-scale tracking. |
...
| recurring automation, recurring digest, threshold tracker, alert, stream watch                                                          | Automation / Push                   | A new user-facing recurring automation defaults to delivery. Read [alva-knowledge.md](references/alva-knowledge.md), then build and verify the full push path in [push-notifications.md](references/push-notifications.md). Preserve a recognized legacy producer only when maintaining an existing feed. |
...
Alerts are personal delivery bindings for automations (feeds). A request to
create a user-facing recurring automation defaults to delivery unless the user
asks for silent or data-only behavior. Playbook follows are independent and
never enable or disable alerts. New feeds declare push-worthy outputs with
`alertOutput(typeDoc)` and may use any valid, non-reserved `group/output`
source. `--push-notify` lets successful scheduled and Run Now executions
deliver those outputs; it does not subscribe users or bypass preferences.
