...
| trade setup — watch a ticker against the user's own trade idea (pullback/breakout/entry plan), "alert me when my setup confirms/breaks", create/edit a trade setup automation, why didn't my trade setup alert, `@alva/trade-setup-sdk`                  | Platform Data: Trade Setup SDK      | Read [trade-setup-sdk.md](references/trade-setup-sdk.md) before any action; follow its entry contract, four-step provisioning, setup_text-only editing, and diagnosis flows. Do not route these to the generic Automation / Push path. |
