...
| explicitly publish/create a Thesis from a viewpoint, update/close/delete an existing Thesis, explicitly polish a viewpoint | Thesis | Read [thesis.md](references/thesis.md). Before `create`, show the final Thesis text and ask the user to confirm it; after a successful authoritative readback, emit one host-native Thesis preview card with verified author, body and tickers; rewriting is a separate explicit action. |
...
For Thesis publication or explicit candidate rewriting, follow [thesis.md](references/thesis.md); before creation, show the final Thesis and accept a natural confirmation of that content. After successful creation and authoritative readback, emit one host-native Thesis preview card with the verified author name/avatar, exact body and resolved tickers. Do not invent a rendering protocol or build HTML, an Automation or a draft/release workflow.
