...
| explicitly publish/create a Thesis from a viewpoint, update/close/delete an existing Thesis, explicitly polish a viewpoint | Thesis | Read [thesis.md](references/thesis.md). Before `create`, show the final Thesis text and ask the user to confirm it; pass the selected original text; rewriting is a separate explicit action, never an automatic publishing step. |
...
For Thesis publication or explicit candidate rewriting, follow [thesis.md](references/thesis.md); before any Thesis creation, show the final Thesis and wait for the user's confirmation of that content; do not build HTML, an Automation or a draft/release workflow.
