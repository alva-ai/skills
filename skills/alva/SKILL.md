...
The directive gives the full catalog id (`<username>/<name>`). Skills live in the playbook-skills catalog on the gateway and are fetched via the `alva skillhub` CLI (use `alva skillhub --help` for the full surface). They can be published by any user — do not assume the `alva/` namespace.
