...
- **Do not hardcode live data into HTML.** Financial metrics (prices, revenue,
  margins, ratios), social content, news, or any data that changes over time
  must be fetched at runtime from feed data paths (`@last`/`@now`), never
  baked into the HTML as literals. If the playbook displays a number, it
  should come from a `fetch()` call at render time.
- **Do not use WebSearch/WebFetch as a data source for playbook content.**
  WebSearch is for understanding requirements, reading documentation, or
  learning about a topic. It is NOT a legitimate data pipeline. Data displayed
  in the playbook must originate from Alva SDK modules, Search API modules, or
  user BYOD via `require("net/http")` — all running inside feeds.
- **Do not create disconnected feeds.** Every deployed feed must be consumed by
  the playbook HTML via `fetch()` calls to the feed's data output paths. A
  feed that runs on schedule but whose output is never read by the playbook is
  a wasted resource and a sign that the data pipeline is broken.
- **Mock / demo playbooks are not exempt.** When a user requests "mock data" or
  a "demo playbook," the feed scripts should generate varied mock data (e.g.
  randomized values, synthetic time series) and the playbook HTML must still
  `fetch()` that data from feed output paths at render time. Do not hardcode
  mock values as JavaScript literals in the HTML. The purpose of mock data is to
  demonstrate the pipeline, not to bypass it. A self-contained HTML file with
  no `fetch()` calls is a static page, not an Alva playbook.
- **Do not generate analysis text from agent knowledge.** Investment summaries,
  bull/bear theses, sentiment analysis, or market outlooks presented as
  playbook content must be generated by feed scripts (e.g. LLM-driven
  transforms on legitimate data), not written by the agent directly into HTML.
- **Do not hardcode values just because no SDK covers them.** If the user's
  request includes data not available from any Alva SDK module, either
  (a) use `require("net/http")` in the feed to fetch from a public API (BYOD),
  (b) omit that widget and explain the limitation, or (c) mark the content as
  a static reference snapshot with a visible label (e.g. "As of FY2026
  earnings report"). Never silently present agent knowledge as live data.
- **Compute KPI aggregates from feed data at render time.** When the playbook
  displays FY totals, trailing averages, or derived metrics (total revenue,
  average margin, cumulative FCF), compute them in the HTML JavaScript from
  quarterly/monthly feed data fetched via `fetch()`. Do not pre-compute and
  hardcode these values — they must update automatically when the feed
  refreshes.
