...
| explicitly publish/create a Thesis from a viewpoint, update/close/delete an existing Thesis, explicitly polish a viewpoint | Thesis | Read [thesis.md](references/thesis.md). Before `create`, show the exact payload and require a clear user confirmation; pass the selected original text; rewriting is a separate explicit action, never an automatic publishing step. |
...
For Thesis publication or explicit candidate rewriting, follow [thesis.md](references/thesis.md); before any Thesis creation, show the exact payload and wait for clear user confirmation; do not build HTML, an Automation or a draft/release workflow.
