...
const casesPath = path.join(root, "evals/alva/skillhub_template_cases.json");

function usage() {
  console.error(
    "Usage: node evals/alva/skillhub_template_eval.js [--label NAME] [--out FILE] [--treeish REF]"
  );
  process.exit(2);
}

let label = "skillhub-template";
let outPath = null;
let treeish = null;
...
const lines = [];
lines.push(`# Alva Skillhub Template Eval — ${sourceLabel}`);
lines.push("");
lines.push(
